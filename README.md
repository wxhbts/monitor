...
> [!NOTE]
> 本项目已全面支持腾讯云 EdgeOne 全球版（中国站与国际站账号均可直接使用）和 阿里云 ESA 和 Cloudflare。
...
4. 在 **环境变量 (Environment Variables)** 中添加以下配置：
   ...
   - `ESASECRET_ID`: 您的阿里云 AccessKey ID
   - `ESASECRET_KEY`: 您的阿里云 AccessKey Secret
   - `CFSECRET_ID`: 您的Cloudflare 账户邮箱
   - `CFSECRET_KEY`: 您的Cloudflare Global API Key
   - `CFZONE_TAG`: 您的Cloudflare 域名区域 ID
   - `CFACCOUNT_TAG`: 您的Cloudflare 账户ID 注：首页邮箱旁 竖排... 复制账户ID
